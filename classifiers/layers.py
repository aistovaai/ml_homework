...
def relu_backward(dout, cache):
    ...
    # Градиент dout там, где x  > 0, 0 там, где x <= 0
    dx = dout * (x > 0)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(H_out):
        for j in range(W_out):
            ...
            # текущее окно
            x_window = x_pad[:, :, hs:hs+HH, ws:ws+WW]  
        ...
        for f in range(F):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    # ошибка в пиксель
                    grad_val = dout[n, f, i, j]
                    
                    x_slice = x_padded[n, :, h_start:h_end, w_start:w_end]
                    
                    dw[f] += grad_val * x_slice
                    dx_padded[n, :, h_start:h_end, w_start:w_end] += grad_val * w[f]
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(H_out):
        for j in range(W_out):
            ...
            # по фильтрам
            for f in range(F):
                df = dout[:, f, i, j].reshape(N, 1, 1, 1)
                dw[f] += np.sum(x_window * df, axis=0)
                dx_pad[:, :, hs:hs+HH, ws:ws+WW] += w[f] * df
        ...
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    flat_idx = np.argmax(x_slice)
                    
                    max_h, max_w = np.unravel_index(flat_idx, x_slice.shape)
                    
                    grad_val = dout[n, c, i, j]
                    dx[n, c, h_start + max_h, w_start + max_w] += grad_val
